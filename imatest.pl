...
# subqueries prepared with binds, innodb status/mut,must longtext, recreate and N tables after destr or every N sec, better SET
...
my @LOPT = ("dry-run!", "$HELP!", "testyaml=s", "$SEE_ALSO=s", "$SEED=i", "$VERBOSE=i", "$VERSION!");
my %HDEFOPT = ("dry-run" => 0, $HELP => 0, $VERBOSE => 0, $VERSION => 0);      # option defaults
...
sub usage {
  ...
  Usage: $EXECUTABLE_NAME $PROGRAM_NAME option...
    ...
    --[no]dry-run optional, run no test if supplied, just check test file syntax
...
sub dosleepms {
    my $slep = $ARG[0];
    return $ghasopt{'dry-run'}? $slep : usleep($slep*1000);
}

# 1: sleep time in seconds # returns: whatever sleep returns or sleep time if --dry-run
sub dosleep {
    my $slep = $ARG[0];
    return $slep if ($ghasopt{'dry-run'});
    my $finfile = "$ghreal{'tmpdir'}/$PID.fin";
    foreach my $i (1..$slep) {
        if (-f $finfile) {
            dosayif($VERBOSE_ANY,"will not sleep %s seconds, return -1 because of %s",$slep,$finfile);
            return -1;
        }
        sleep(1);
    }
...
sub checkscript {
    ...
    # check subkeys
    if (defined($ghver{'_2levelkeys'})) {
        my %hcheck = ();
        foreach my $allowed (@{$ghver{'_2levelkeys'}}) {
            ++$hcheck{$allowed};
        }
        my %hsub = ();
        foreach my $key (sort(keys(%ghver))) {
            next if ($key eq '_2levelkeys');
            my $ph = $ghver{$key};
        ...
        foreach my $have (sort(keys(%hsub))) {
            docroak("%s='%s' is defined in %s '%s' but '%s' is not there. CROAK.",
                    '_2levelkeys',"@{$ghver{'_2levelkeys'}}", 'to_check_file', $ghtest{'to_check_file'}, $have)
...
sub db_discover {
    ...
    if ($ghasopt{'dry-run'}) {
        dosayif($VERBOSE_ANY,"with %s=%s returning %s", 'dry-run', 1,  $rc);
        return $rc;
    }

    @glschemas = () if ($strict != 2);
    my $com = "SHOW SCHEMAS";
    ...
    push(@glschemas, qw(sys mysql performance_schema information_schema)) if ($strict == 2);
...
sub new_generate_virtual {
    ...
    for my $termnum (1..$tlen) {
        ...
        if ($tkind eq 'value') {
            ...
            $tfun = new_postprocess_fun($tfun,$tnam,$pluse,0);
            $tval = $tfun;
        }
...
sub new_generate_lob_value {
    ...
    $rc = substr($rc,0,6553) if (length($rc) > 6553);
    my $len = length($rc);
    $rc .= substr($rc,0,$lun);
...
sub new_generate_insert_values {
    ...
    foreach my $col (@$plcols) {
        ...
        if ($kind eq 'function') {
            ...
            my $subrc = process_rseq($parm);
            $subrc = new_postprocess_fun($subrc,$tnam,$plcols,0);
            $rc .= ", $subrc";
            next;
        }
        if ($kind eq 'value') {
...
sub new_generate_default {
    ...
    if ($kind eq 'column') {
        ...
        $rc = new_postprocess_fun($rc,$tnam,\@lc,0);
        $rc = "($rc)";
    } elsif ($kind eq 'value') {
...
# 1: dbh 2: absport # 3: stmt # 4: key name string # returns hashref or undef
sub gethashref {
...
# 1: stmt # 2: dbh # 3: verbosity on error # returns RC_OK or RC_ERROR
sub runreport {
...
# add schema.table to global structures # 1: schema.table which is expected to exist # 2: db handle # 3: croak on error
sub table_add {
...
sub new_get_column {
    my ($tnam,$plmuse,$typ,$strict) = @ARG;
    $strict = 0 if (not defined($strict));
    docroak("no plmuse for tnam '%s' typ '%s'",$tnam,$typ) if (scalar(@$plmuse) == 0);
    my @luse = @{$plmuse};
    @luse = doshuffle(@luse);
    my $pluse = \@luse;
    my $tcol = '';
...
sub db_create {
    ...
    docroak("Failed to db_discover before table creation, rc=%s",$rc) if ($rc != $RC_OK);
    ...
    my %hs2ntables = ();
    my @lschemas = grep {$_ ne 'imaschema' and $_ ne 'sys' and $_ ne 'mysql' and not $_ =~ /^(performance|information)_schema$/} @glschemas;
    #docroak("#debug+%s+%s+","@lschemas","@glschemas");
    foreach my $snam (@lschemas) {
...
sub stmt_select_generate {
    ...
    if ($joins > 1) {
        ...
        foreach my $i (2..$joins) {
            my $jtable = dorand() < $ghreal{'select_join_sametable_p'}? table_get(1) : $tnam;
            push(@ltjoins,$jtable);
    ...
    foreach my $jtnam (@ltjoins) {
        ++$nj;
        if ($havejoins and $nj > 1 and not dorand() < process_rseq('select_join_where_table_p')) {
            next;
        } else {
            $wher .= " $op ";
        }
...
sub server_load_thread {
    ...
    sub table_columns_subset {
        ...
        docroak("no havejoins") if (not defined($havejoins));
        if (not defined($ghst2nvcols{$tnam})) {
            my $canint = $kind eq 'select'? 1 : 0;
            my $newtnam = table_get($canint);
...
# 6: subtype eq same ksql or ALTER_DROP etc
    ...
    while (1) {
        ...
        if ($ksql eq 'select') {
            ...
            my $frm = process_rseq("table_name_format");
            my @lschemas = grep {$_ ne 'imaschema' and $_ ne 'sys' and $_ ne 'mysql' and not $_ =~ /^(performance|information)_schema$/} @glschemas;
            my $schema = $lschemas[int(dorand()*scalar(@lschemas))];
        ...
        my $errstr;
        my $rows;
        docroak("Suspicios stmt wrt mysql schema: +%s+",$stmt) if ($stmt =~ /mysql\./i and not $stmt =~ /^\s*(explain\s+(analyze\s+)?|check\s+|show\s+parse_tree\s+)?\s*SELECT/i and not $stmt =~ /^\s*SHOW\s+COLUMNS\s+FROM\s+/i);
        if ($dosql) {
...
sub new_terminate_servers {
    ...
    dosayif($VERBOSE_ANY," execution of %s resulted in exit code %s for step %s",$howout,$subec,$stepnum);
    do_sql_every_step(); #todo ports maybe
    return;
}
...
sub start_load_thread {
    ...
    my $rseed = $ARG[2];
    my $rc = $RC_OK;
    dosayif($VERBOSE_ANY, " invoked with --%s=%s",  'dry-run', $ghasopt{'dry-run'});
    if (not $ghasopt{'dry-run'}) {
        my $pid = fork();
        if ($pid == 0) {
            use DBI;
...
sub do_sql_every_step {
    my @lports = map {$_+$ghreal{'mportoffset'}} split(/,+/,$ghreal{'sql_every_step_ports'});
    my @lcnf = split("\n",$ghreal{'sql_every_step'});
    foreach my $absport (@lports) {
        if (not defined($ghdbh{$absport})) {
            my $adbh;
            $ghdbh{$absport} = doconnect(\$adbh,$absport,1,0);
        }
        foreach my $stmt (@lcnf) {
            dosayif($VERBOSE_ANY,"port %s handle %s to run %s",$absport,$ghdbh{$absport},$stmt);
            runreport($stmt,$ghdbh{$absport},$absport,$VERBOSE_ANY);
        }
    }
}

# rc RC_OK: initialised successfully #    RC_ZERO: nothing to do
sub init_db {
    my $rc = $RC_OK;
    my $subrc;
    dosayif($VERBOSE_ANY,"invoked");
    $rc = $RC_ZERO if $ghasopt{'dry-run'};
